...
export function AppHeader() {
  ...
  const { getNotifications, markNotificationAsRead } = useAppData();
  const unreadNotifications = getNotifications(false);
            ...
            <ScrollArea className="h-[300px]">
              {getNotifications().length === 0 ? (
                ...
                getNotifications().map((notif) => (
                  <DropdownMenuItem key={notif.id} onClick={() => markNotificationAsRead(notif.id)} className={`flex flex-col items-start gap-1 whitespace-normal ${!notif.read ? 'bg-accent/50' : ''}`}>
